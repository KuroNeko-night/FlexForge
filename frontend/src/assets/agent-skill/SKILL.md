...
## 7.5 AI 助手业务适配（建议，P30）

平台 AI 助手经内置工具读取业务实体元数据（`list_entities` / `inspect_entity`），
回答"有哪些业务/某业务有哪些字段"类问题。你的插件**无需写任何代码**即可被覆盖——
实体一经激活即自动进入助手可见的业务实体索引；适配要点全部在元数据质量：

- `displayName` 用业务语言（"采购订单"而非"po"）——这是助手引用你业务时的名字；
- 每个字段都给 `displayName`（"单号"而非 order_no）——助手按它转述字段结构；
- 实体名/字段名保持 `snake_case` 语义清晰——助手工具按 name 查询、按 displayName 呈现。
