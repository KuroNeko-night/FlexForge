...
## 7.5 AI 助手业务适配（建议，P30-P31）

平台 AI 助手经内置工具读取业务实体元数据与真实记录（`list_entities` / `inspect_entity`
/ `query_records`），回答"有哪些业务/某业务有哪些字段/某笔记录什么情况"类问题。你的
插件**无需写任何代码**即可被覆盖——实体一经激活即自动进入助手可见的业务实体索引；
适配要点全部在元数据质量：

- `displayName` 用业务语言（"采购订单"而非"po"）——这是助手引用你业务时的名字；
- 每个字段都给 `displayName`（"单号"而非 order_no）——助手按它转述字段结构、
  按它呈现查询到的记录内容；
- 实体名/字段名保持 `snake_case` 语义清晰——助手工具按 name 查询（含记录过滤的
  field 参数）、按 displayName 呈现；
- 记录内容是用户自己录入的数据——助手只读查询（按创建时间倒序、条数与长度有限额），
  不会写入或删除任何记录。

以上做不到不会导致校验失败，但会显著降低 AI 助手对你业务的问答质量——
面向使用者的插件建议按此自查。平台工具面（`AssistantTool`/`WorkshopTool`）
为平台内置扩展位，插件侧不提供可执行代码（声明式边界不变）。
