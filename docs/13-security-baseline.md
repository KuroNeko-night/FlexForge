...
11. **助手业务数据查询边界（P31，FR-KB-08）**：`query_records` 只读——唯一取数路径为既有 `DynamicRecordService.query`（service.data-access 公开契约），不新增 SQL 面；过滤字段/操作符/类型兼容复用 DataQueryParams 白名单解析（NFR-SEC-02 参数化口径不变），启用实体之外一律按不存在处理；输出预算硬截断（每次工具调用 ≤10 条、单字段值 ≤60 字符、总输出超预算截断并标注省略）防以问答为放大器的数据拖库；记录可见性与 `/api/v1/data` 查询一致（任意已认证角色本可读，助手不放宽也不收紧）；工具结果仍为数据段回灌（同第 2/10 条口径），记录内容不进日志与审计；写/删操作不在助手工具面（工具接口只读，无预留写位）。
