...
9. **需求工坊工具调用边界（P30，FR-ISSUE-09）**：模型只在提示词协议内"调用"工具——输出为待校验数据而非可执行代码；工具面由服务端 `WorkshopTool` 接口白名单收口（当前仅 create_issue，Spring 注入注册），未知工具按非法输出重试后 400；create_issue 的 title/description 复用既有创建校验（≤120/≤4000 等），creator=认证用户（模型不得代表他人），审计走既有 issue.create/saveSpec；工坊对话为数据段注入（同第 2 条口径），模板 workshop-v1 声明对话内容非指令；模型失败不落半截会话（与 FR-KB-04 同口径）。
10. **助手业务工具边界（P30，FR-KB-07）**：工具只读（MetaRegistry 查询），白名单为平台内置 AssistantTool Bean 注册表；模型输出的工具调用是"待校验数据"——参数非法/未知工具以错误文本回灌提示词（数据段）不执行不中断；调用次数（每问 ≤2）与模型轮次硬上限；工具结果不落日志与审计；实体索引为平台生成的只读数据段，插件作者无代码适配面（displayName 质量即适配，见开发 Skill §7.5）。
