...
> - 输出预算：每次工具调用 ≤10 条、单字段值 ≤60 字符、总输出超预算截断并标注（防数据拖库）；记录可见性=/api/v1/data 查询（已认证可读，不放宽）；工具结果仍数据段回灌、不进日志审计（S7/§3.6-11）；写/删不在助手工具面；
...
## P32：工坊输入区固定与 per-issue 详情分区重排（2026-09-18 用户验收反馈裁决新增）

> 背景：P31 验收反馈两点——①AI 助手输入区已固定，但工坊界面输入区不固定（空态/短历史时悬在列中部，与助手页底部固定不同构）；②per-issue 详情（澄清对话/确认卡/讨论区）松散平铺在滚动列里，块间只有间距无边界，滚动后失去上下文，"排版太乱不好理解"。
> **红线**：
> - 工坊对话与 AI 助手同构：消息流（或空态引导）flex 拉伸填满、输入区恒在工坊列底部视口内（NFR-UX-02 补全，工坊列不产生外层滚动）；
> - per-issue 详情分区卡片化：sticky 头部（返回工坊+标题+状态+描述，滚动不丢上下文）+「需求澄清」卡片（内部自滚）+ 确认卡/已发布提示 +「讨论区」卡片；分区标题与卡片边界提供视觉锚点；
> - 服务端契约零变化（纯前端呈现层重排）；语义/测试锚点（data-testid）保持稳定；i18n 新增键走语言包升版流程；
> - 极简风格基线不变（卡片=surface+border+radius 令牌，不引入新装饰）。
> **非目标**：不做双栏详情（保持单列分区滚动）；不改澄清/讨论组件内部行为（只包壳与布局）。

### 实施内容

- **A. 工坊输入区固定**：IssueWorkshopChat 空态/加载态 flex 拉伸（`.chat-empty`/`.state` flex:1），composer 恒落工坊列底。
- **B. 详情分区重排**：IssueWorkshopView per-issue 模板重构——`.detail` 滚动容器内：`.detail-head`（sticky 顶部+背景遮盖）→「需求澄清」section 卡片（IssueClarifyChat 整体内嵌、内部自滚或自然高度）→ 确认卡/已发布提示 →「讨论区」section 卡片（IssueDiscussion 内嵌）；`.main` 溢出收敛（工坊自管、详情 `.detail` 自管）。
- **测试**：IssueWorkshopView.test.ts 既有 7 例回归（testid 不变）；布局浏览器走查（长对话输入区固定+详情视觉）。

### 验收标准

- 工坊任意消息量（含空态）输入区恒在视口底部、消息流内滚、外层不产生滚动。
- per-issue 详情：sticky 头部滚动可见；澄清/确认/讨论三区分组清晰有边界；滚动上下文不丢。
- 前端 229/229、门禁 21/1/0、CI 六绿；服务端契约与既有测试锚点零变化。
