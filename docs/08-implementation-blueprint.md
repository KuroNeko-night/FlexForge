...
统一错误码至少包括：`invalid_manifest`、`unsupported_schema_version`（P07 schemaVersion 分派层）、`dependency_missing`、`migration_failed`、`registration_failed`、`activation_not_found`、`stale_activation`、`permission_denied`、`validation_error`、`not_found`、`internal_error`、`unauthorized`（后四类为 P02 统一错误装配与 P03 认证引入，代码见 `flexforge-common` `ErrorCodes`）、`invalid_transition`（P10 Issue 状态机：非法迁移/缺原因/缺合法规格批准门）、`model_unavailable`（P11 模型超时/限流/离线，503）、`model_output_invalid`（P11 模型输出重试超限，400 可走手工规格）。`
