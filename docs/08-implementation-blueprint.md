...
统一错误码至少包括：`invalid_manifest`、`dependency_missing`、`migration_failed`、`registration_failed`、`activation_not_found`、`stale_activation`、`permission_denied`、`validation_error`、`not_found`、`internal_error`、`unauthorized`（后四类为 P02 统一错误装配与 P03 认证引入，代码见 `flexforge-common` `ErrorCodes`）。
