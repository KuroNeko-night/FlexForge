...
│   ├── flexforge-kb/                知识库与 AI 助手（P28，P29-P30 扩）：kb_entry/kb_chat_message/kb_attachment 仓储+KbRetrieval 共现佐证 Top-K+KbAttachments 附件提取（docx/xlsx zip+XML、pdf PDFBox、炸弹防护）+KbAssistantService/AssistantToolLoop（ModelPort 唯一通道、kb-assistant-v3 工具环路）+AssistantTool 业务检查（list/inspect 经 meta）+条目 CRUD/ask/附件下载 API（写仅 ADMIN）
