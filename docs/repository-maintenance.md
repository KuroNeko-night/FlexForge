...
- 数据库结构只能通过编号迁移变更；根 `database/migrations/` 只允许平台骨架迁移（`sys_*`/`meta_*`/`data_*`/`plugin_*`/`issue_*` 及 `issue`/`requirement_spec`，`data_*` 为动态记录存储 2026-08-24 P05 定案；`issue*`/`requirement_spec` 为 P10 Issue 域平台表，`ai_*` 为 P11 AI 任务记录表，`processor_*` 为 P23 处理器产物表），业务表迁移必须位于插件包内并由插件安装流程执行（ADR-0004）。
