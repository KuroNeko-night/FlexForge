...
| `extension.data-processor` | `{key, label, kind: 'python', entry, inputEntity}`；entry 指向包内 `scripts/*.py`（S6 Level 2 校验），inputEntity 为目标实体名。执行契约：stdin `{records:[{...data}]}` → stdout `{kind:'table', columns:[{name,label}], rows:[[...]]}`、`{kind:'summary', items:[{label,value}]}` 或 `{kind:'chart', chartType:'bar'|'pie', title, categories:[..], values:[..]}`（平台 Schema 校验；chart：categories ≤50 项每项 ≤100 字符、values 有限数字等长 ≤50、pie 值 ≥0，P22）；子进程 `python3 -I`+超时+IO/行数上限=平台单页 200 行（ADR-0002 Level 2 受信边界）。invoke 与动态数据读同权（ADMIN/USER） | flexforge-plugin（注册表+执行引擎）+ 前端实体页分析抽屉 | plugins/example-analytics（月度透视/合格率/负载汇总，P20） | P20 | active |
